                ...
                for t_sub in subtimesteps:
                    if t_sub > 0:
                        ...
                        for i in range(n_cascade_levels):
                            ...
                            # Make sure that there are numbers in the extrapolation
                            # component, otherwise the extrapolator returns an error.
                            # If there are only nans in the extrapolation component,
                            # fill it up with the minimum value present in R_f_ip. The
                            # mask will be applied later on in the extrapolation step.
                            if np.any(np.isfinite(R_f_ip[i])) == False:
                                R_f_ip[i][~np.isfinite(R_f_ip[i])] = np.nanmin(R_f_ip)
